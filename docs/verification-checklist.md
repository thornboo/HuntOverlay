...
> macOS 侧已验证：186 单元测试全过、整条 import 链 offscreen 成立。
...
- [ ] **菜单位置与尺寸**：控制中心启动时位于主屏幕中央，默认为横向窗口，可自由缩放且不遮挡透明地图覆盖层
- [ ] **功能导航**：左侧「地图与工具 / 官方点位 / 首领资料 / 设置」均可切换，当前页状态清楚，键盘 Tab 可到达
- [ ] **全选 / 全不选**：「官方点位」页两个按钮能一键开启/关闭所有 POI 分类，叠加层同步变化
- [ ] **地图译名**：下拉框显示「静水河口 / 劳森三角洲 / 德萨莱 / 玛门峡谷」
- [ ] 切地图、改颜色、调缩放、快捷键设置都正常（重构后行为不变）
- [ ] 在 Windows 100% / 125% / 150% DPI 下检查文字、下拉箭头、数字框箭头和按钮边框无裁切
- [ ] 多显示器环境下从托盘恢复或用快捷键打开时，控制中心仍完整落在主屏幕可用区域
...
| 结果 | 含义 | 下一步 |
|------|------|------|
| ✅ 点位对上了 | 纯数据陈旧，A+B 已解决 | 继续验证编辑点位、标尺、图片缓存 |
...
- [ ] 在 POIs 页先选择「点位类型」
- [ ] 点「编辑点位」后不弹出 modal，叠加层直接进入选点状态
- [ ] 左键点击地图后立即新增一个用户点位，`%LOCALAPPDATA%\HuntOverlay\user_pois.json` 被写入
- [ ] 不退出选点状态时，继续左键可以连续新增多个点位
- [ ] 右键或 Esc 退出选点状态，覆盖层恢复正常点击穿透
- [ ] 重启程序后新增点位仍然存在

## 6. 标尺（持续多标尺）

- [ ] 点「标尺」后，左键设置起点，再左键设置终点
- [ ] 第一条标尺完成后不会消失，继续左键可画下一条
- [ ] 鼠标移到任一标尺端点时显示删除图标，左键该图标可删除该条标尺
- [ ] 点「清空标尺」会一次清掉所有已画标尺
- [ ] 右键或 Esc 退出标尺模式后，已画标尺仍保留显示

## 7. 图片缓存

- [ ] 首次启动或手动刷新数据后，程序会在后台预下载缺失的 POI 图片
- [ ] 预下载完成后，hover 有图片的点位应直接从本地缓存显示，不再等待在线加载
- [ ] 刚刷新后若立刻 hover 到尚未预下载完成的图片，会短暂显示「正在加载图片...」，随后该图片进入本地缓存
- [ ] 关闭网络后，已缓存过的图片仍可显示

## 8. POI 渲染合并（可选，验证 user_pois 这一层）
...
3. 第 5/6/7 项编辑点位、标尺、图片缓存是否符合预期
4. 第 9 项语言切换是否完整（哪处切到英文后仍是中文）
